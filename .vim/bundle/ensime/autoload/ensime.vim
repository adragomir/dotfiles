...
let s:c['ensime-script'] = get(s:c, 'ensime-script', expand('<sfile>:h:h:h').'/dist/bin/server')
let s:c['ensime-script'] = get(s:c, 'ensime-script', expand('<sfile>:h:h:h').'/dist/bin/server')
...
fun! ensime#StartEnsimeServer()
  ...
  fun! ctx.got_port(data)
    ...
    call s:Log("COMMAND: " .substitute(s:c.nc, 'PORT', port, 'g'))
...
fun! ensime#Connect()
  call s:Log("ensime connect called fuckingly !")
